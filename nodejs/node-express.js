...
//responde hola cuando se solicita la carpeta raiz
app.get("/",function(request,response){
	response.send("hola");
});
//acepta un parametro :text
app.get("/hola/:text",function(request,response){
	response.send("Hola "+request.params.text);//para acceder a la variable :text se hace request.params.text
});
var users ={
	"1":{
		"name":"Pepe Lopez",
